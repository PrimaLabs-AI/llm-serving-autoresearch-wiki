...
### The core idea: autoresearch, specialized to TPU perf

The methodology is an **autoresearch loop** — after [Karpathy's autoresearch paper](raw/code/autoresearch), specialized to TPU performance optimization. At any moment the LLM holds a **ranked queue of falsifiable hypotheses** — *"swap XLA SDPA for splash attention"*, *"enable `use_fused_bwd_kernel`"*, *"try batch=3 now that selective remat freed HBM"*, *"push splash's block_q = block_kv to 1024 at seq=1024"*. Each is scored by expected gain × confidence / effort. The agent picks the top candidate, translates it into a minimal code diff against the current best, runs a short benchmark on real hardware, captures an xprof trace + HLO dump, and compares op-by-op against the prior profile. The outcome is a **profile-grounded verdict**: `-accepted` if the delta clears the noise band with no regression on the loss trajectory or other tracked metrics, `-rejected` if refuted, `-potential` if flat but worth parking. Every verdict lands as a verdict-suffixed markdown writeup, a row in a machine-readable ledger, and a named git branch — so a human can audit, reproduce, or roll back to any prior state at any time.

**Revised priors feed the next round**, which is what makes the loop compound. A refuted hypothesis narrows its cousins (when `latency_hiding_scheduler` alone was flat, the loop pinpointed the bundle regression to the `async_collective_fusion_*` family). An accepted hypothesis unlocks its memory-dependent dependents (selective remat freed HBM → batch=3 became tractable → batch amortized per-step fixed costs → fused-bwd landed cleanly on top). Generalizable observations — *"Pallas kernels only beat XLA when XLA isn't already fusing the pattern"*, *"scan-over-layers on TPU requires an internally-tiled attention kernel"*, *"per-chip batch has a mechanistic sweet spot at the HBM ceiling"* — get promoted to concept and analysis pages that survive across model and hardware changes, quietly making every future run smarter.

The formalism is what makes it actually work. Without the **verdict → ledger → next-hypothesis** discipline, an LLM-driven optimizer tends to oscillate, re-try refuted ideas, or cargo-cult optimizations without evidence. With it, the system gets monotonically better at the problem, because every experiment — win or loss — permanently improves the priors for the next one. What the human curates is the target, the noise band, and the occasional contradiction; the LLM owns the drudgework of proposing, running, profiling, and writing up.
