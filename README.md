...
The methodology is an **autoresearch loop** — after [Karpathy's autoresearch paper](https://github.com/karpathy/autoresearch), specialized to TPU performance optimization. At any moment the LLM holds a **ranked queue of falsifiable hypotheses** (e.g. *"swap XLA SDPA for splash attention"*, *"try batch=3 now that selective remat freed HBM"*), each scored by expected gain × confidence / effort. It picks the top one, applies a minimal code diff, runs a short benchmark on real hardware, captures an xprof trace + HLO dump, and diffs it against the prior best to produce a **profile-grounded verdict** — `-accepted`, `-rejected`, or `-potential` — written to a markdown page, a ledger row, and a named git branch. Revised priors feed the next round: refuted hypotheses narrow their cousins, accepted ones unlock memory-dependent dependents, and generalizable observations get promoted to analysis pages that survive across model and hardware changes. The discipline is what makes it compound — without it, LLM-driven optimizers oscillate and cargo-cult; with it, every experiment permanently improves the priors for the next one.
