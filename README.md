...
Raw XProf is a web UI, not something an LLM agent can use directly. We bridge the gap with [**xprof_mcp**](https://github.com/openxla/xprof-mcp), an [MCP](https://modelcontextprotocol.io/) server that turns XProf into a set of programmatic tools the LLM calls directly — `list_runs`, `get_overview`, `get_top_hlo_ops`, `get_op_profile`, `get_memory_profile`, `get_hlo_dump_neighborhood`, and more. The LLM asks *"which op is hot?"* and gets a structured, ranked answer; *"why is it memory-bound?"* returns FLOPs / bytes / arithmetic-intensity numbers against the v6e ridge point; *"where's the HBM peak coming from?"* returns a per-buffer breakdown of the live-set at peak.

The decisive capability is **backtracing through the compiler**. A hot op in a TPU profile is typically a **fused HLO** — a compiler-synthesized blob with no direct analogue in the model source. `xprof_mcp` also exposes the XLA HLO dumps (captured when the trainer is launched with `XLA_FLAGS="--xla_dump_to=<dir> --xla_dump_hlo_as_text"`), so the LLM can walk from the hot op in the **optimized HLO** back through XLA's passes — layout assignment, fusion, scheduling, collective-fusion, rematerialization — to the **unoptimized HLO** the framework emitted, and from there into the ingested framework and model source code (see the wiki + source-ingestion components below). That chain is what lets the LLM say, concretely: *"this hot kernel came from this line of `modeling_gemma4.py`; the fusion-boundary break is XLA's scheduler reordering the matmul away from its consumer; the fix is a sharding constraint at this site."* Without the chain, TPU perf work is guesswork. With it, the LLM can reason mechanistically about every op in the trace.
