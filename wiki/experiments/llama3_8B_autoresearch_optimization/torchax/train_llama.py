...
def main(
  ...
  if tpu_num_slices == 1:
    # axis_types=Auto: JAX infers output shardings for ops where they can be
    # determined unambiguously (gather/scatter/etc). Newer JAX defaults to
    # Explicit which requires manual out_sharding annotations on every op
    # — torchtitan model code wasn't written for that and breaks at the
    # embedding gather. Auto matches the canonical example's intent.
  ...
  # torchtitan 0.2.0 registers `freqs_cis` as a non-persistent buffer (not in
  # state_dict). The canonical example targeted an older torchtitan where it
  # was persistent. We defensively pop and explicitly re-attach freqs_cis as
  # a buffer on the live model after sharded weights are loaded.
  state_dict = dict(gpt.state_dict())
  state_dict.pop("freqs_cis", None)  # safe whether present or not
  state_dict = create_sharded_weights(gpt, mesh, sharding_map)
  replicated = jax.sharding.NamedSharding(mesh, P())

  gpt.load_state_dict(state_dict, assign=True, strict=False)
  # Attach freqs_cis directly on the model (replicated across all devices).
