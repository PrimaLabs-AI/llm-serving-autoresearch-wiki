...
- [Llama 3 8B — TPU autoresearch optimization](experiments/llama3_8B_autoresearch_optimization/README.md) — program page for `meta-llama/Meta-Llama-3-8B` (and Llama 3.1-8B) on TPU v6e via torchax / native-JAX / MaxText. Status: **🚀 EXCEEDED MaxText reference (+5.7 %) — [JAX exp 18: native-JAX (Flax NNX) + scan + tokamax CE + tokamax-splash with `bkv=2048` (matched MaxText) + MaxText XLA flag stack (HOST_OFFLOAD scheduler bundle + DISABLE_COLLECTIVE_MATMUL + SparseCore offload) at bs=5 seq=8192 = 59,765 TPS (7,471/chip), 41.9 % reported MFU](experiments/llama3_8B_autoresearch_optimization/jax/experiments/2026-04-26-jax-exp13-maxtext-xla-stack-bs5-accepted.md#follow-up-2026-04-27-match-maxtexts-exact-splash-config)** vs MaxText reference 7,069/chip = **+5.7 % throughput per chip**. (Reported MFU gap is FLOP-counter formula attribution differences between trainers; real throughput we exceed.) **Cumulative climb 2026-04-26/27: 4,591 → 7,471 tok/s/chip = +62.7 %** vs the morning AMP-only torchax baseline (exp 20). Confirmed: tokamax-splash with `use_base2_exp + fuse_reciprocal + max_logit_const` is the **+4.4 %** delta vs MaxText's kernel choice (exp 20 with jax-experimental splash → 7,142/chip ≈ MaxText's 7,138). Torchax frontier 2026-04-26: [exp 74b 6,559/chip 36.8 % MFU at bs=3 seq=8192](experiments/llama3_8B_autoresearch_optimization/torchax/experiments/2026-04-26-exp72a-tokamax-splash-bs3-seq8k-accepted.md#follow-up--max_logit_const-exp-74). Key wins: (1) tokamax CE (chunked_xla impl) unlocks bs=3+ at seq=8192; (2) tokamax-shipped splash with the perf knobs; (3) MaxText's HOST_OFFLOAD scheduler-flag bundle (`xla_tpu_enable_all_experimental_scheduler_features`, `xla_latency_hiding_scheduler_rerun=2`, etc.) — the breakthrough lever on the JAX stack; (4) splash `bkv=2048` matched to MaxText; (5) bs=5 fits with memory pressure tracking enabled (we don't host-offload activations like MaxText does, so we pack denser HBM).
