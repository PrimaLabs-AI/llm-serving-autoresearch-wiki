...
*Last updated: 2026-04-26 — 191 pages (27 codebases + 45 sources + 96 concepts + 2 model-programs + 4 analyses + 6 analysis subpages + jax-exp47 rejected page + 2 maxtext baseline experiments + 1 torchax llama3-8b baseline + 1 torchax llama3-8b splash kernel-only autotune)*
...
- [Llama 3 8B — TPU autoresearch optimization](experiments/llama3_8B_autoresearch_optimization/README.md) — program page for `meta-llama/Meta-Llama-3-8B` (and Llama 3.1-8B) on TPU v6e via torchax / native-JAX / MaxText. Status: **🏆 PROGRAM-TARGET BEST 2026-04-26 — [exp 74b: scan + chunked_xla CE + tokamax-splash (`use_base2_exp + fuse_reciprocal + max_logit_const=30`) + fp32 master + bf16 compute @ bs=3 seq=8192 = 52,475 TPS (6,559/chip), 36.8 % MFU](experiments/llama3_8B_autoresearch_optimization/torchax/experiments/2026-04-26-exp72a-tokamax-splash-bs3-seq8k-accepted.md#follow-up--max_logit_const-exp-74)**. Cumulative climb today: 4,591 → 6,559 tok/s/chip = **+42.9 %** vs the morning AMP-only baseline (exp 20). Frontier waypoints: exp 55b 6,178; exp 56 6,202; exp 62b 6,303; exp 65 6,313; exp 72a 6,392; exp 74b 6,559. Gap to MaxText 44.6 % reference: 7.8 pp. Note: exp 66 series (bf16-native chunked_xla without fp32 cast) **invalidated** — kernel accumulates lse in input dtype, so bf16 input compromises loss precision (loss plateaued at multiples of 0.0625 vs the valid stack's clean monotone decay). — **+35 % per-chip TPS over exp 20** (the prior seq=8192 best at 31.6 % MFU bs=1). Earlier non-program-target peak: exp 9 splash-autotuned bs=4 seq=1024 = 7,225/chip, 36.1 % MFU. MaxText reference (Llama 3.1-8B) at 44.6 % MFU is the ceiling. New 2026-04-26 wins: (5) **tokamax `linear_softmax_cross_entropy_loss` (mosaic_tpu) kernel via `shard_map`** unlocks bs=3 (and bs=4) at seq=8192 — was OOM by 6 GiB without it; (6) **fp32 master weights with bf16 compute via `_maybe_cast_weights` autocast** — true canonical AMP master, free at runtime (cast fused into matmul); (7) **MFU formula bug fixed** — `avg_step_time` was dividing by planned step count instead of actually-measured count, inflating MFU 1.3-1.9× when the wikitext loader exited early (corrected exp 51b/c, 53b numbers in writeup). Also from 2026-04-25: (1) splash + bs=4 = +12.8 pp MFU; (2) kernel-only autotune found +30 % fwd+bwd kernel win, +0.4 pp MFU end-to-end; (3) `nothing_saveable` remat refuted (52 % step-time tax + still OOM); (4) `torchax.train.make_train_step`'s `remat_policy` arg is silently ignored.
