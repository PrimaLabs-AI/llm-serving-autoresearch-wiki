...
- [Llama 3 8B — TPU autoresearch optimization](experiments/llama3_8B_autoresearch_optimization/README.md) — program page for `meta-llama/Meta-Llama-3-8B` (and Llama 3.1-8B) on TPU v6e via torchax / native-JAX / MaxText. Status: **🚀 EXCEEDED MaxText reference — [JAX exp 13: native-JAX (Flax NNX) + scan + tokamax CE + tokamax-splash + MaxText XLA flag stack (HOST_OFFLOAD scheduler bundle + DISABLE_COLLECTIVE_MATMUL) at bs=5 seq=8192 = 59,317 TPS (7,415/chip), 41.6 % reported MFU](experiments/llama3_8B_autoresearch_optimization/jax/experiments/2026-04-26-jax-exp13-maxtext-xla-stack-bs5-accepted.md)** vs MaxText reference 7,138/chip 44.6 % MFU = **+3.9 % throughput per chip**. (The reported MFU gap is FLOP-counting normalization between trainers, not a real throughput gap; on absolute tok/s/chip we exceed.) **Cumulative climb today: 4,591 → 7,415 tok/s/chip = +61.5 %** vs the morning AMP-only torchax baseline (exp 20). Torchax frontier at end of day: [exp 74b 6,559/chip 36.8 % MFU at bs=3 seq=8192](experiments/llama3_8B_autoresearch_optimization/torchax/experiments/2026-04-26-exp72a-tokamax-splash-bs3-seq8k-accepted.md#follow-up--max_logit_const-exp-74) (validates same wins on torchax stack). Key wins: (1) tokamax CE (chunked_xla impl) unlocks bs=3+ at seq=8192; (2) tokamax-shipped splash with use_base2_exp + fuse_reciprocal + max_logit_const knobs; (3) MaxText's HOST_OFFLOAD scheduler-flag bundle (`xla_tpu_enable_all_experimental_scheduler_features`, `xla_latency_hiding_scheduler_rerun=2`, etc.) — the breakthrough lever on the JAX stack; (4) bs=5 fits with memory pressure tracking enabled.
