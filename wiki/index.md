...
*Last updated: 2026-04-25 — 189 pages (27 codebases + 45 sources + 96 concepts + 2 model-programs + 4 analyses + 6 analysis subpages + jax-exp47 rejected page + 2 maxtext baseline experiments + 1 torchax llama3-8b baseline + 1 torchax llama3-8b splash kernel-only autotune)*
...
- [Llama 3 8B — TPU autoresearch optimization](experiments/llama3_8B_autoresearch_optimization/README.md) — program page for `meta-llama/Meta-Llama-3-8B` (and Llama 3.1-8B) on TPU v6e via torchax / native-JAX / MaxText. Status: **torchax v6e-8 best 2026-04-25 (after 11 experiments): exp 9 splash-autotuned + bs=4 = 57,799 TPS (7,225/chip), 36.1 % MFU** — +57.4 % TPS / +13.2 pp MFU vs baseline 22.9 %. MaxText reference (Llama 3.1-8B) at 44.6 % MFU is the ceiling (**8.5 pp gap remaining**). Run state: 1 baseline + 3 accepted + 3 rejected + 2 potential + 3 invalid-OOM. Big findings: (1) splash + bs=4 = +12.8 pp MFU; (2) kernel-only autotune found +30 % fwd+bwd kernel win, +0.4 pp MFU end-to-end; (3) `nothing_saveable` remat refuted (52 % step-time tax + still OOM); (4) `torchax.train.make_train_step`'s `remat_policy` arg is silently ignored (line commented out in canonical source). Next-priority unlocks for fp32-master + seq=8192 program target: scan-over-layers (collapse 32-layer unroll), selective remat (`dots_saveable`), tokamax CE kernel, host-offload of optimizer state.
